...
function normalizeTrade(trade) {
  ...
  const pnlPoints =
    typeof trade.pnl === "number" ? trade.pnl : (trade.pnl?.points ?? 0);
  ...
  return {
    ...
    lifecycle: trade.lifecycle ?? trade.status ?? null,
    ...
    result:
      trade.result || (pnlPoints > 0 ? "WIN" : pnlPoints < 0 ? "LOSS" : null),
  };
}
...
function normalizeStats(stats, trades = []) {
  ...
  const total =
    stats?.total ?? stats?.totalTrades ?? normalizedTrades.length ?? 0;
  const wins =
    stats?.wins ??
    stats?.winners ??
    normalizedTrades.filter((trade) => trade.result === "WIN").length;
  const losses = stats?.losses ?? stats?.losers ?? Math.max(0, total - wins);
  const totalPnl = stats?.totalPnl ?? 0;
  const avgPnl = stats?.avgPnl ?? (total > 0 ? totalPnl / total : 0);
  const maxDrawdown =
    stats?.maxDrawdown ?? computeMaxDrawdown(normalizedTrades);
...
function createControllers(strategyRunner, journal) {
  ...
  router.get("/option-quote", async (req, res) => {
    try {
      ...
      const grade = String(
        req.query.grade || req.query.signalGrade || "B+",
      ).toUpperCase();
      ...
      if (!smartApi) {
        return res.json({
          status: "UNAVAILABLE",
          reason: "Option data provider unavailable",
          strike,
          optionType,
          grade,
          optionEntry: null,
      ...
      const quote = await fetchAngelOneOptionQuote(
        smartApi,
        strike,
        optionType,
        req.query.tradingSymbol,
      );
      if (!quote?.ltp) {
        return res.json({
          status: "WAITING_QUOTE",
          reason: "Live option quote not found yet",
          strike,
          optionType,
          grade,
          optionEntry: null,
  ...
  router.post("/trade/stop-loss", async (req, res) => {
    ...
    const updatedTrade =
      strategyRunner.pnlTracker.adjustStopLoss(requestedStopLoss);
    if (!updatedTrade) {
  ...
  router.post("/trade/close", async (req, res) => {
    ...
    const closedTrade = {
      ...
      status: "CLOSED",
      pnl:
        dbOpenTrade.type === "CALL"
          ? Math.round(
              (lastPrice -
                (dbOpenTrade.entry?.price ?? dbOpenTrade.entry ?? 0)) *
                100,
            ) / 100
          : Math.round(
              ((dbOpenTrade.entry?.price ?? dbOpenTrade.entry ?? 0) -
    ...
    closedTrade.pnlPercent =
      closedTrade.entry?.price || closedTrade.entry
        ? Math.round(
            (closedTrade.pnl /
              (closedTrade.entry?.price ?? closedTrade.entry)) *
              10000,
    ...
    await dbService.updateTrade(dbOpenTrade._id, {
      ...
      closeReason: "MANUAL",
      "metadata.duration":
        closedTrade.closeTime -
        (dbOpenTrade.entry?.time
          ? new Date(dbOpenTrade.entry.time).getTime()
          : new Date(dbOpenTrade.createdAt || Date.now()).getTime()),
    });
  ...
  router.get("/journal/stats", async (req, res) => {
    try {
      ...
      const persistedTrades = (
        await dbService.getTradesWithFilter({}, 200)
      ).map(normalizeTrade);
      const persistedStats = await dbService.getTradeStatsWithFilter({});
  ...
  router.post("/test-telegram", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to send alert", details: error.message });
    }
  });
  ...
  router.get("/db/market-data", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to load market data from database" });
    }
  });
